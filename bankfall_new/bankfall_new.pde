...
void update() {
  ...
  for (int i = 0; i < data.getRowCount(); i++) {
    ...
    if (row.getString("Closing Date").contains(searchYear)) {
      ...
      // add ball
      balls[i] = new Ball(random(width), random(height), random(30, 70), i, balls, name, close);
      numBalls++;
    }
  }
  println("total: " + total);
...
// below here is modified BouncyBubbles example code
...
class Ball {
  ...
  Ball(float xin, float yin, float din, int idin, Ball[] oin, String bankName, String closeDate ) {
    x = xin;
    y = yin;
    diameter = din;
    id = idin;
